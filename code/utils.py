...
def obtain_label(loader, net, device):
    start_test = True
    net.eval()
    predict = np.array([], dtype=np.int64)

    with torch.no_grad():
        iter_test = data_prefetcher(loader)
...
def extract_embeddings(model, dataloader):
    ...
    for _, images, target, _, _ in dataloader:
        with torch.no_grad():
            images = images.cuda()
