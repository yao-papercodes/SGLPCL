...
class FixedThresholdingHook():
    ...
    def masking(self, logits_x_ulb, softmax_x_ulb=True, p_cutoff=0.99, *args, **kwargs):
        if softmax_x_ulb:
            print(f"logits_x_ulb[0] = {logits_x_ulb[0]}")
            probs_x_ulb = torch.softmax(logits_x_ulb.detach(), dim=-1)
            print(f"probs_x_ulb[0] = {probs_x_ulb[0]}")
        else:
...
def gen_ulb_targets(logits, use_hard_label=True, T=2.0, softmax=True, label_smoothing=0.0): # softmax: whether to compute softmax for logits, input must be logits
    """
...
@torch.no_grad()
def get_plabels_confi_dist(key_feats, value_feats, value_probs, topk=3):
    '''
    inputs:
        key_feats: (bs, dims).
        values_feats: (n, dims), n is totol numbers of domain datasets.
        value_probs: (n, c), c is the classes number.
    return:
        pseudo_labels: (bs, ).
        confidences: (bs, ).
        mmd_dist: (bs, 1).
    '''
    bs = key_feats.size(0)
    device = key_feats.device
    classes = value_probs.size(1)
    dists = torch.cdist(key_feats, value_feats, p=2) # (bs, n)
    top_idxs = torch.topk(dists, k=topk, dim=1, largest=False).indices # (bs, topk)
    probs = torch.mean(value_probs[top_idxs], dim=1) # (bs, c)
    pseudo_labels = probs.max(-1)[1] # (bs, )
    confidences = torch.exp(-Entropy(probs)/torch.log2(torch.tensor(classes))) # (bs, )
    top_value_feats = value_feats[top_idxs] # (bs, topk, dims)
    mmd_dist = torch.zeros(bs, 1).to(device) # (bs, 1)
    for i in range(bs):
        mmd_dist[i] = compute_mmd(key_feats[i].unsqueeze(0), top_value_feats[i])
    
    return pseudo_labels, confidences, mmd_dist

@torch.no_grad()
def get_dist_label(key_feats, value_feats, value_probs, value_labels, topk=3):
    '''
    inputs:
        key_feats: (bs, dims).
        values_feats: (n, dims), n is totol numbers of domain datasets.
    ...
    top_idxs = torch.topk(dists, k=topk, dim=1, largest=False).indices # (bs, topk)
    top_value_feats = value_feats[top_idxs] # (bs, topk, dims)
    top_value_labels = value_labels[top_idxs] # (bs, topk, 1)
    mmd_dist = torch.zeros(bs, 1).to(device) # (bs, 1)
    for i in range(bs):
        mmd_dist[i] = compute_mmd(key_feats[i].unsqueeze(0), top_value_feats[i])
    probs = torch.mean(value_probs[top_idxs], dim=1) # (bs, c)
    pseudo_labels = probs.max(-1)[1] # (bs, )
    vote_labels = vote_for_label(top_value_labels)

    return mmd_dist, pseudo_labels, vote_labels
...
def nl_criterion(output, y):
    ...
    labels_neg = ( (y.unsqueeze(-1).repeat(1, 1) + torch.LongTensor(len(y), 1).random_(1, class_num).to(device)) % class_num ).view(-1)

    l = F.nll_loss(output, labels_neg, reduction='none')

    return l

# Function to perform voting strategy
def vote_for_label(tensor):
    # Initialize an empty tensor for the final labels
    final_labels = torch.empty(tensor.size(0), dtype=torch.long).to(tensor.device)

    for i in range(tensor.size(0)):
        # Count occurrences of each label
        unique, counts = torch.unique(tensor[i], return_counts=True)
        ...
        # If there's a tie, select randomly among the candidates
        final_labels[i] = candidates[torch.randint(len(candidates), (1,))]
