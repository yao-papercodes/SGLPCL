...
def open_file(dataset):
    ...
    if ext == '.mat':
        ...
        if 'HyRANK' in dataset:
            return io.loadmat(dataset)  # HyRANK数据集用这个
        elif 'Houston' in dataset:
            return h5py.File(dataset)   # Houston数据集用这个
        elif 'Pavia' in dataset:
            return io.loadmat(dataset)  # Pavia数据集用这个
        elif 'S-H' in dataset:
            return io.loadmat(dataset)  # S-H数据集用这个
...
def sample_gt(gt, train_size, mode='random'):
    ...
    if mode == 'random':
        if train_size == 1: # target data
            ...
            test_set = np.column_stack((test_indices[0],test_indices[1],test_label)) # 这些构造够可以参考上面的if那里
    ...
    '''
    gt、train_gt、train_set的区别:
    gt: (W, H), 每个pixel都有类别包括0
    train_gt: (W, H), gt的基础上只有被选为训练集的pixel才有类别标签
    train_set: (train_num, 3), train_num是被选为训练集的总的pixel总量, 每个元素是一个三元组(w, h, c)即横纵坐标和对应的这个pixel的类别
    '''
...
def getGraphdataOneDomain(share, bs):
    segments = torch.reshape(torch.tensor(range(bs)),(-1, int(math.sqrt(bs)))) # segments的shape是(sqrt(10), sqrt(10))
    edge = torch.tensor(getEdge(share, segments)).t().contiguous() # src_edge.shape (2, 595), 记录的是这个batch里边每个样本的边的关系
    share_graph = Data(x=share, edge_index=edge).to(DEVICE)
    return share_graph

def getGraphdataCrossDomain(src_share, tar_share, bs):
    edge = torch.tensor(getEdgeCrossDomain(bs)).t().contiguous()
    share_graph = Data(x=torch.concat((src_share, tar_share), dim=0), edge_index=edge).to(DEVICE)
    return share_graph

def getEdgeCrossDomain(bs):
    coo = set()
    for src_i in range(bs):
        for tar_i in range(bs, 2*bs):
            coo.add((src_i, tar_i))
            coo.add((tar_i, src_i))
...
def getEdge_ClassGuided(src_labels, tar_labels, mask):
    ...
    for i in range(src_node):
        if src_labels[i].item() not in src_dict:
...
def trans_one_hot(labels, classes):
    if labels.dim == 1:
        labels = labels.unsqueeze(1)
    y_one_hot = torch.zeros(labels.size(0), classes).to(labels.device)
    return y_one_hot.scatter_(1, labels.unsqueeze(1).to(dtype=torch.int64), 1)

epsilon = 1e-6
def cal_entropy(predictions, flag='src'):
    class_num = predictions.size(1)
    if flag == 'src':
        probabilities = torch.clamp(predictions, epsilon, 1 - (class_num - 1) * epsilon)
    elif flag == 'tar':
        probabilities = F.softmax(predictions, dim=1)
        probabilities = torch.clamp(probabilities, epsilon, 1.0 - (class_num - 1) * epsilon)
    elif flag == 'cross':
        bs = int(predictions.size(0)/2)
        src_probabilities = torch.clamp(predictions[:bs], epsilon, 1 - (class_num - 1) * epsilon)
        tar_probabilities = F.softmax(predictions[bs:], dim=1)
        tar_probabilities = torch.clamp(tar_probabilities, epsilon, 1.0 - (class_num - 1) * epsilon)
        probabilities = torch.concat((src_probabilities, tar_probabilities))
    entropy_values = -torch.sum(probabilities * torch.log2(probabilities), dim=1).unsqueeze(1)    
    return probabilities, cent_entropy(entropy_values, class_num, flag)

def cent_entropy(entropy, class_num, flag):
    device = entropy.device
    max_dist = torch.tensor([1/class_num for i in range(class_num)]).to(device)
    min_dist = torch.tensor([0 for i in range(class_num-1)] + [1]).to(device)
    min_dist = torch.clamp(min_dist, epsilon, 1.0 - (class_num - 1) * epsilon)
    max_entropy = -torch.sum(max_dist * torch.log2(max_dist))
    min_entropy = -torch.sum(min_dist * torch.log2(min_dist))
    return (max_entropy - entropy) / (max_entropy - min_entropy)

def compute_mmd(source_samples, target_samples, sigma=1.0):
    # 计算源域样本的核矩阵
    source_kernel = gaussian_kernel(source_samples, source_samples, sigma)
    
    # 计算目标域样本的核矩阵
    target_kernel = gaussian_kernel(target_samples, target_samples, sigma)
    
    # 计算源域样本和目标域样本之间的核矩阵
    cross_kernel = gaussian_kernel(source_samples, target_samples, sigma)
